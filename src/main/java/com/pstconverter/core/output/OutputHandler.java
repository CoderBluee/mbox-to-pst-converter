...
public interface OutputHandler {
    ...
    static java.util.List<String> saveAttachments(File targetFolder, MailMessage msg, int index, String safeSubject, String attachmentHandling) {
        ...
        String prefixPath = "";
        if (attachmentHandling.equalsIgnoreCase("Keep Attachments in Folder")) {
    ...
    static String buildMimeMessageString(MailMessage msg) {
        ...
        if (attachments == null || attachments.isEmpty()) {
            ...
            sb.append(getPlainTextBody(msg.getBody())).append("\n");
            return sb.toString();
        }
    ...
    static String getPlainTextBody(String body) {
        ...
        if (lower.contains("<html") || lower.contains("<body") || lower.contains("<p") || lower.contains("<br") || lower.contains("<div") || lower.contains("<style") || lower.contains("<!--")) {
            // Strip HTML comments
            body = body.replaceAll("(?s)<!--.*?-->", "");
            
            // Strip head/style/script tags and their contents completely
            body = body.replaceAll("(?s)<style\\b[^>]*>.*?</style>", "");
            body = body.replaceAll("(?s)<script\\b[^>]*>.*?</script>", "");
            body = body.replaceAll("(?s)<head\\b[^>]*>.*?</head>", "");
            
            // Replace <br> and <br/> and close tags with newlines
            body = body.replaceAll("(?i)<br\\s*/?>", "\n");
            body = body.replaceAll("(?i)</?(p|div|tr|h[1-6]|li)\\b[^>]*>", "\n");
