...
rect = new org.apache.pdfbox.pdmodel.common.PDRectangle(rect.getHeight(), rect.getWidth());
    ...
    private void drawText(PDPageContentStream contentStream, org.apache.pdfbox.pdmodel.font.PDFont font, float fontSize, float[] color, String text, float x, float y) throws Exception {
        ...
        contentStream.showText(OutputHandler.sanitizePdfText(text));
    ...
    private void handlePdfAttachments(PDDocument document, MailMessage msg, File targetFolder, int index, String safeSubject, String attachmentHandling) {
        String pdfAttachMode = com.pstconverter.util.SettingsManager.getSetting("pdf_attachment_mode", "Embed directly into output file");
        boolean shouldEmbed;
        boolean shouldSaveLocally;
        
        if (pdfAttachMode.equalsIgnoreCase("Ignore attachments")) {
            shouldEmbed = false;
            shouldSaveLocally = false;
        } else if (pdfAttachMode.equalsIgnoreCase("Extract and save in separate folder next to output")) {
            shouldEmbed = false;
            shouldSaveLocally = true;
        } else if (pdfAttachMode.equalsIgnoreCase("Embed directly into output file")) {
            shouldEmbed = true;
            shouldSaveLocally = false;
        } else {
            // Fallback to the method argument passed from UI if anything else
            shouldEmbed = attachmentHandling != null && attachmentHandling.contains("Embed Attachments");
            shouldSaveLocally = !shouldEmbed || attachmentHandling.contains("Keep Attachments") || attachmentHandling.contains("Separate Attachment");
        }
    ...
    private MailMessage getSanitizedMessageCopy(MailMessage original) {
        MailMessage copy = new MailMessage(
            ...
            OutputHandler.sanitizePdfText(original.getBody(), true),
