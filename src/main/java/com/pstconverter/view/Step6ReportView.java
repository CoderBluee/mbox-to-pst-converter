...
public class Step6ReportView extends BorderPane {
    ...
    private void initializeUI() {
        ...
        Button btnOpenOutputBanner = new Button("Open Folder");
        Label iconOutputBtn = MaterialIcons.icon(MaterialIcons.FOLDER_OPEN);
        iconOutputBtn.setStyle("-fx-font-size: 15px; -fx-text-fill: white;");
        btnOpenOutputBanner.setGraphic(iconOutputBtn);
        btnOpenOutputBanner.getStyleClass().addAll("action-btn", "btn-primary");
        btnOpenOutputBanner.setStyle("-fx-background-color: linear-gradient(to right, #4f46e5, #06b6d4); -fx-text-fill: white; -fx-font-size: 11.5px; -fx-padding: 7px 14px; -fx-font-weight: bold; -fx-background-radius: 20px; -fx-border-radius: 20px; -fx-cursor: hand;");
        btnOpenOutputBanner.setMinWidth(Region.USE_PREF_SIZE);
        btnOpenOutputBanner.setOnAction(e -> openOutputFolder());

        Button btnDownloadPdf = new Button("PDF Report");
        Label iconPdfBtn = MaterialIcons.icon(MaterialIcons.PICTURE_AS_PDF);
        iconPdfBtn.setStyle("-fx-font-size: 15px; -fx-text-fill: white;");
        btnDownloadPdf.setGraphic(iconPdfBtn);
        btnDownloadPdf.getStyleClass().addAll("action-btn", "btn-primary");
        btnDownloadPdf.setStyle("-fx-background-color: linear-gradient(to right, #6366f1, #3b82f6); -fx-text-fill: white; -fx-font-size: 11.5px; -fx-padding: 7px 14px; -fx-font-weight: bold; -fx-background-radius: 20px; -fx-border-radius: 20px; -fx-cursor: hand;");
        btnDownloadPdf.setMinWidth(Region.USE_PREF_SIZE);
        btnDownloadPdf.setOnAction(e -> handleDownloadPdfReport());

        Button btnOpenSession = new Button("Session Logs");
        ...
        String secBtnStyle = "-fx-background-color: " + (isDark ? "rgba(255, 255, 255, 0.06)" : "rgba(0, 0, 0, 0.04)") + "; -fx-text-fill: " + (isDark ? "#c7d2fe" : "#4f46e5") + "; -fx-border-color: " + (isDark ? "rgba(99, 102, 241, 0.3)" : "rgba(79, 70, 229, 0.2)") + "; -fx-border-width: 1px; -fx-font-size: 11.5px; -fx-padding: 7px 14px; -fx-font-weight: bold; -fx-background-radius: 20px; -fx-border-radius: 20px; -fx-cursor: hand;";
        btnOpenSession.setStyle(secBtnStyle);
        btnOpenSession.setMinWidth(Region.USE_PREF_SIZE);
        ...
        btnConvertAgain.setMinWidth(Region.USE_PREF_SIZE);
        ...
        addSummaryTile(statsGrid, 0, MaterialIcons.CHECK_CIRCLE, "Converted", "—", "#34d399", "converted");
        addSummaryTile(statsGrid, 1, MaterialIcons.BLOCK, "Skipped", "—", "#64748b", "skipped");
        addSummaryTile(statsGrid, 2, MaterialIcons.WARNING, "Failed", "—", "#f87171", "failed");
        addSummaryTile(statsGrid, 3, MaterialIcons.SCHEDULE, "Duration", "—", "#a78bfa", "time");
        addSummaryTile(statsGrid, 4, MaterialIcons.DATA_USAGE, "Export Size", "—", "#38bdf8", "output");
        ...
        metricsTitle.setStyle("-fx-font-size: 11.5px; -fx-font-weight: 800;");
        ...
        paramsTitle.setStyle("-fx-font-size: 11.5px; -fx-font-weight: 800;");
        ...
        treeTitle.setStyle("-fx-font-size: 11.5px; -fx-font-weight: 800;");
                ...
                if (empty || item == null) {
                    ...
                    switch (status) {
                        ...
                        case "FILE" -> {
                            ...
                            iconL.setStyle("-fx-text-fill: #818cf8; -fx-font-size: 14px;");
                            textStyle = "-fx-text-fill: " + (isDarkCurrent ? "#f8fafc" : "#312e81") + "; -fx-font-weight: bold; -fx-font-size: 13px;";
                        }
                        case "PENDING" -> {
                            iconL = MaterialIcons.icon(MaterialIcons.FOLDER, 13);
                            iconL.setStyle("-fx-text-fill: #94a3b8; -fx-font-size: 13px;");
                            textStyle = "-fx-text-fill: " + (isDarkCurrent ? "#cbd5e1" : "#475569") + "; -fx-font-size: 13px;";
                        }
                        default -> {
                            iconL = MaterialIcons.icon(MaterialIcons.FOLDER, 13);
        ...
        skipsTitle.setStyle("-fx-font-size: 11.5px; -fx-font-weight: 800;");
    ...
    private void addSummaryTile(GridPane grid, int col, String iconCodepoint, String caption, String value, String colorHex, String id) {
        ...
        Label iconLabel = MaterialIcons.icon(iconCodepoint, "-fx-font-size: 18px; -fx-text-fill: " + colorHex + ";");
        ...
        valL.setStyle("-fx-font-size: 22px; -fx-font-weight: 800; -fx-text-fill: " + colorHex + ";");
        ...
        tile.setOnMouseEntered(e -> {
            tile.setScaleX(1.02);
            tile.setScaleY(1.02);
            tile.setEffect(new javafx.scene.effect.DropShadow(12, javafx.scene.paint.Color.web(colorHex, 0.25)));
        });
    ...
    public void loadReportData() {
        ...
        lblSummaryConverted.setStyle("-fx-font-size: 24px; -fx-font-weight: 800; -fx-text-fill: " + colorSuccess + ";");
        lblSummarySkipped.setStyle("-fx-font-size: 24px; -fx-font-weight: 800; -fx-text-fill: " + colorSkipped + ";");
        lblSummaryFailed.setStyle("-fx-font-size: 24px; -fx-font-weight: 800; -fx-text-fill: " + colorFailed + ";");
        lblSummaryTime.setStyle("-fx-font-size: 24px; -fx-font-weight: 800; -fx-text-fill: " + colorTime + ";");
        lblSummaryOutput.setStyle("-fx-font-size: 24px; -fx-font-weight: 800; -fx-text-fill: " + colorSize + ";");
        ...
        String btnSecColor = isDark ? "#c7d2fe" : "#4f46e5";
        ...
        if (step3 != null) {
            ...
            for (String filter : activeFilters) {
                ...
                chip.setStyle("-fx-background-color: " + (isDark ? "rgba(99, 102, 241, 0.15)" : "rgba(79, 70, 229, 0.08)") + "; -fx-background-radius: 12px; -fx-border-color: " + (isDark ? "rgba(99, 102, 241, 0.3)" : "rgba(79, 70, 229, 0.15)") + "; -fx-border-radius: 12px;");
                
                Label dot = MaterialIcons.icon(MaterialIcons.BOLT, "-fx-font-size: 12px; -fx-text-fill: " + (isDark ? "#818cf8" : "#4f46e5") + ";");
                Label text = new Label(filter);
                text.setStyle("-fx-font-size: 11.5px; -fx-font-weight: bold; -fx-text-fill: " + (isDark ? "#f8fafc" : "#1e1b4b") + ";");
