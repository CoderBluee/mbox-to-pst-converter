...
public class UserMappingDialog extends Dialog<Map<String, String>> {
    ...
    public UserMappingDialog(List<SourceFileModel> fileList, String defaultAdminEmail, Map<String, String> existingMapping, List<String> domainUsers) {
        ...
        boolean isDark = "Dark Mode".equalsIgnoreCase(com.pstconverter.util.SettingsManager.getSetting("ui_theme", "Dark Mode"));

        // Header Title
        VBox headerBox = new VBox(4);
        Label titleLbl = new Label("User Mailbox Mapping Matrix");
        titleLbl.setStyle("-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: " + (isDark ? "#ffffff" : "#1e293b") + ";");
        
        String subText = "Map each imported Mailbox file to its target domain user account for Admin Impersonation.";
        ...
        subtitleLbl.setStyle("-fx-font-size: 11px; -fx-text-fill: " + (isDark ? "#94a3b8" : "#64748b") + ";");
        ...
        tableView.setStyle("-fx-background-color: " + (isDark ? "#131b2e" : "#ffffff") + "; -fx-border-color: " + (isDark ? "#334155" : "#e2e8f0") + "; -fx-border-radius: 6px;");
