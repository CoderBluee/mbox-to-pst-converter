...
public class Step3FilterView extends BorderPane {
    ...
    public void updateFilterSummary() {
        ...
        if (chips.isEmpty()) {
            ...
            noFilters.setStyle("-fx-font-size: 11px; -fx-text-fill: " + (controller != null && controller.isDarkMode() ? "#cbd5e1" : "#64748b") + "; -fx-font-style: italic;");
            ...
            for (String chipText : chips) {
                ...
                chip.setStyle(
                    "-fx-background-color: " + (isDark ? "rgba(99, 102, 241, 0.25)" : "rgba(99, 102, 241, 0.12)") + "; " +
                    "-fx-text-fill: " + (isDark ? "#c7d2fe" : "#4f46e5") + "; " +
                    "-fx-font-size: 10.5px; -fx-font-weight: bold; -fx-padding: 3px 9px; -fx-background-radius: 10px; " +
                    "-fx-border-color: " + (isDark ? "rgba(129, 140, 248, 0.4)" : "rgba(99, 102, 241, 0.2)") + "; -fx-border-radius: 10px;"
                );
    ...
    private void handleLoadSendersPopup(TextField targetField) {
        ...
        progressRoot.setStyle("-fx-background-color: #1e293b; -fx-border-color: #4f46e5; -fx-border-width: 2;");
    ...
    private HBox createCompactSummaryBar() {
        ...
        Label filterIcon = MaterialIcons.icon(MaterialIcons.SEARCH, "-fx-font-size: 15px; -fx-text-fill: " + (isDark ? "#818cf8" : "#4f46e5") + ";");
        compactActiveCount = new Label("0 active");
        compactActiveCount.getStyleClass().add("count-badge");
        compactActiveCount.setStyle(
            "-fx-background-color: " + (isDark ? "rgba(99, 102, 241, 0.2)" : "rgba(99, 102, 241, 0.12)") + "; " +
            "-fx-text-fill: " + (isDark ? "#a5b4fc" : "#4f46e5") + "; " +
            "-fx-font-size: 11px; -fx-font-weight: bold; -fx-padding: 3px 10px; -fx-background-radius: 12px;"
        );
        ...
        String scanBaseStyle = "-fx-background-color: linear-gradient(to right, #4f46e5, #06b6d4); " +
                               "-fx-text-fill: white; -fx-font-size: 11px; -fx-font-weight: bold; " +
                               "-fx-padding: 5px 12px; -fx-background-radius: 12px; -fx-cursor: hand;";
        btnScan.setStyle(scanBaseStyle);

        lblScanStatus = new Label("");
        lblScanStatus.setStyle("-fx-font-size: 11.5px; -fx-text-fill: " + (isDark ? "#818cf8" : "#4f46e5") + "; -fx-font-weight: bold;");
    ...
    private VBox createCategoryNavCard(String iconCode, String title, String subtitle, int index) {
        ...
        switch (index) {
            ...
            case 1: iconColor = isDark ? "#818cf8" : "#4f46e5"; break; // Date Range (Indigo)
        ...
        subtitleLabel.setStyle("-fx-font-size: 11.5px; -fx-text-fill: " + (isDark ? "#cbd5e1" : "#64748b") + "; -fx-padding: 0 0 0 24px;");
    ...
    public void selectCategory(int index) {
        ...
        for (int i = 0; i < categoryNavBox.getChildren().size(); i++) {
            ...
            if (i == index) {
                ...
                card.setStyle("-fx-background-color: " + (isDark ? "rgba(99, 102, 241, 0.18)" : "rgba(99, 102, 241, 0.08)") + "; " +
                              "-fx-border-color: " + (isDark ? "#818cf8" : "#4f46e5") + "; " +
    ...
    public void refreshAllStyles() {
        ...
        if (hygienePanel != null) {
            hygienePanel.refreshStyles();
        }
        if (itemTypePanel != null) {
            itemTypePanel.refreshStyles();
        }
    }
    private void showPlaceholder() {
        ...
        Label placeholderIcon = MaterialIcons.icon(MaterialIcons.SETTINGS, 48);
        placeholderIcon.setStyle("-fx-text-fill: #818cf8;");
    ...
    private VBox createPresetsSection() {
        ...
        HBox header = new HBox(8);
        header.setAlignment(Pos.CENTER_LEFT);
        Label iconHeader = MaterialIcons.icon(MaterialIcons.STAR, 16);
        iconHeader.setStyle("-fx-text-fill: #fbbf24;");
        Label titleHeader = new Label("Saved Filter Presets");
        ...
        HBox saveHeaderRow = new HBox(8);
        saveHeaderRow.setAlignment(Pos.CENTER_LEFT);
        Label saveIcon = MaterialIcons.icon(MaterialIcons.SAVE, 15);
        saveIcon.setStyle("-fx-text-fill: #818cf8;");
        Label lblSaveHeader = new Label("Save Current Configuration");
        lblSaveHeader.getStyleClass().add("filter-group-label");
        lblSaveHeader.setStyle("-fx-font-size: 13px; -fx-text-fill: #818cf8; -fx-font-weight: bold;");
        saveHeaderRow.getChildren().addAll(saveIcon, lblSaveHeader);
        ...
        saveCard.getChildren().addAll(saveHeaderRow, new Separator(), saveRow);
        ...
        HBox listHeaderRow = new HBox(8);
        listHeaderRow.setAlignment(Pos.CENTER_LEFT);
        Label listIcon = MaterialIcons.icon(MaterialIcons.NOTE, 15);
        listIcon.setStyle("-fx-text-fill: #818cf8;");
        Label lblListHeader = new Label("Available Presets");
        lblListHeader.getStyleClass().add("filter-group-label");
        lblListHeader.setStyle("-fx-font-size: 13px; -fx-text-fill: #818cf8; -fx-font-weight: bold;");
        listHeaderRow.getChildren().addAll(listIcon, lblListHeader);

        presetsListContainer = new VBox(10);
        listCard.getChildren().addAll(listHeaderRow, new Separator(), presetsListContainer);
    ...
    private void refreshPresetsList() {
        ...
        if (presets.isEmpty()) {
            ...
            lblEmpty.setStyle("-fx-font-size: 12px; -fx-text-fill: " + (isDark ? "#cbd5e1" : "#64748b") + "; -fx-font-style: italic;");
        ...
        for (java.util.Map.Entry<String, String> entry : presets.entrySet()) {
            ...
            if (activeSummaries.isEmpty()) {
                ...
                lblNoF.setStyle("-fx-font-size: 10px; -fx-text-fill: " + (isDark ? "#cbd5e1" : "#64748b") + "; -fx-font-style: italic;");
