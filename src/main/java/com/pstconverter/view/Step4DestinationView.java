...
public class Step4DestinationView extends BorderPane {
    ...
    private void initializeUI() {
        ...
        headerRow.setStyle(
            ...
            "-fx-border-color: " + (isDark ? "rgba(99, 102, 241, 0.25)" : "rgba(99, 102, 241, 0.15)") + "; " +
        ...
        iconHeader.setStyle("-fx-text-fill: " + (isDark ? "#818cf8" : "#4f46e5") + ";");

        VBox titleBox = new VBox(2);
        Label headerTitle = new Label("CONVERT & EXPORT CONFIGURATION");
        headerTitle.getStyleClass().add("card-header-title");
        headerTitle.setStyle("-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: " + (isDark ? "#c7d2fe" : "#1e1b4b") + ";");
        ...
        // 2. Category Radio Cards Ã¢â‚¬â€ full-width, icon-rich selectable tiles
        HBox categoryBar = new HBox(10);
        categoryBar.setAlignment(Pos.CENTER_LEFT);
        categoryBar.setPadding(new Insets(2, 0, 6, 0));

        cardDocs   = buildCategoryCard("Document Formats",  "HTML, PDF, DOC, JSON...",  com.pstconverter.util.MaterialIcons.DESCRIPTION, "DOCUMENTS", "#3b82f6");
        cardEmails = buildCategoryCard("Email File Formats", "EML, MBOX, MSG, PST...",   com.pstconverter.util.MaterialIcons.EMAIL,        "EMAILS",    "#4f46e5");
        cardCloud  = buildCategoryCard("Cloud Migrations",   "Gmail, Office 365, IMAP...", com.pstconverter.util.MaterialIcons.CLOUD_SYNC,   "CLOUD",     "#10b981");
    ...
    private VBox buildCategoryCard(String title, String subtitle, String iconCode, String categoryId, String accentColor) {
        ...
        titleLabel.setStyle("-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: " + (dark ? "#ffffff" : "#1e293b") + ";");
        titleLabel.setId("cat-title-" + categoryId);

        Label subtitleLabel = new Label(subtitle);
        subtitleLabel.setStyle("-fx-font-size: 11px; -fx-text-fill: " + (dark ? "#cbd5e1" : "#64748b") + ";");
    ...
    public void selectCategory(String categoryId) {
        ...
        applyCategoryCardStyle(cardDocs,   "DOCUMENTS".equals(categoryId), "#3b82f6");
        applyCategoryCardStyle(cardEmails, "EMAILS".equals(categoryId),    "#4f46e5");
        ...
        for (String fmt : formatsToRender) {
            ...
            Label lblName = new Label(getFormatDisplayName(fmt));
    ...
    private void applyCategoryCardStyle(VBox card, boolean active, String accentColor) {
        ...
        if (active) {
            ...
            card.setStyle("-fx-background-color: " + bg + "; " +
                          ...
                          "-fx-border-radius: 10px; -fx-background-radius: 10px; -fx-cursor: hand;");
            if (titleLbl != null)
                titleLbl.setStyle("-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: " + (dark ? "#ffffff" : accentColor) + ";");
            if (subtitleLbl != null)
                subtitleLbl.setStyle("-fx-font-size: 11px; -fx-text-fill: " + (dark ? "#cbd5e1" : accentColor) + ";");
            if (arrowLbl != null)
                arrowLbl.setStyle("-fx-font-size: 14px; -fx-text-fill: " + (dark ? "#818cf8" : accentColor) + ";");
        } else {
            String bg = dark ? "#151d30" : "#ffffff";
            String border = dark ? "rgba(255,255,255,0.08)" : "#e2e8f0";
            card.setStyle("-fx-background-color: " + bg + "; " +
                          "-fx-border-color: " + border + "; -fx-border-width: 1.5px; " +
                          "-fx-border-radius: 10px; -fx-background-radius: 10px; -fx-cursor: hand;");
            if (titleLbl != null)
                titleLbl.setStyle("-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: " + (dark ? "#cbd5e1" : "#1e293b") + ";");
            if (subtitleLbl != null)
    ...
    private void applyFormatCardStyle(String format, boolean active) {
        ...
        if (active) {
            card.setStyle("-fx-background-color: " + (dark ? "rgba(99, 102, 241, 0.22)" : "rgba(79, 70, 229, 0.08)") + "; " +
                          "-fx-border-color: " + (dark ? "#818cf8" : "#4f46e5") + "; -fx-border-width: 1.5px; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-cursor: hand;");
            ((Label)card.getChildren().get(1)).setStyle("-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: " + (dark ? "#f8fafc" : "#4f46e5") + ";");
        } else {
            card.setStyle("-fx-background-color: " + (dark ? "#151d30" : "#ffffff") + "; " +
                          "-fx-border-color: " + (dark ? "rgba(255,255,255,0.08)" : "#e2e8f0") + "; " +
    ...
    public void refreshAllStyles() {
        applyCategoryCardStyle(cardDocs, "DOCUMENTS".equals(selectedCategory), "#3b82f6");
        applyCategoryCardStyle(cardEmails, "EMAILS".equals(selectedCategory), "#4f46e5");
        applyCategoryCardStyle(cardCloud, "CLOUD".equals(selectedCategory), "#10b981");
        for (java.util.Map.Entry<String, HBox> entry : formatCardsMap.entrySet()) {
            applyFormatCardStyle(entry.getKey(), entry.getKey().equals(selectedFormat));
        }
        if (settingsPanelExtracted != null) {
            settingsPanelExtracted.refreshStyles();
        }
    }
    ...
    private String getFormatDisplayName(String format) {
        switch (format) {
            ...
            case "MHTML": return "MHTML MIME";
            case "TXT": return "Plain Text";
            case "RTF": return "Rich Text RTF";
            case "DOC": return "MS Word DOC";
            case "JSON": return "JSON Data";
            case "CSV": return "CSV Spreadsheet";
            case "MBOX": return "MBOX Archive";
            case "EML": return "EML Message";
            case "MSG": return "Outlook MSG";
            case "PST": return "Outlook PST";
            case "EMLX": return "Apple Mail EMLX";
            case "Office 365": return "Office 365";
            case "Gmail": return "Gmail / G Suite";
            case "IMAP Server": return "IMAP Server";
            case "Yahoo Mail": return "Yahoo Mail";
    ...
    private void renderSettingsForm() {
        ...
        lblDestinationValidationError.getStyleClass().add("dest-validation-error");
    ...
    public void loadDestinationSettings(Properties props, String destinationPath) {
        ...
        if (!isCloudFormat(fmt)) {
            ...
            // Local path is restored by `SettingsManager` in `MainController` anyway.
        } else {
