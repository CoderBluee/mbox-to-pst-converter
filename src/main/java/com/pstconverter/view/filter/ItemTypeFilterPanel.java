...
public class ItemTypeFilterPanel extends VBox {
    ...
    private HBox createItemTypeCard(String icon, String title, String description, MFXCheckbox cb) {
        ...
        Runnable updateStyle = () -> {
            ...
            if (isDark) {
                iconContainer.setStyle("-fx-background-color: " + (selected ? "rgba(99, 102, 241, 0.2)" : "rgba(255,255,255,0.06)") + "; -fx-background-radius: 6px;");
                card.setStyle("-fx-background-color: " + (selected ? "rgba(99, 102, 241, 0.15)" : "rgba(255,255,255,0.02)") + "; -fx-border-color: " + (selected ? "#6366f1" : "rgba(255,255,255,0.08)") + "; -fx-border-width: 1px; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-cursor: hand;");
                ...
                iconContainer.setStyle("-fx-background-color: " + (selected ? "rgba(79, 70, 229, 0.1)" : "rgba(0,0,0,0.04)") + "; -fx-background-radius: 6px;");
                card.setStyle("-fx-background-color: " + (selected ? "rgba(79, 70, 229, 0.05)" : "rgba(0,0,0,0.01)") + "; -fx-border-color: " + (selected ? "#4f46e5" : "rgba(0,0,0,0.08)") + "; -fx-border-width: 1px; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-cursor: hand;");
                titleLabel.setStyle("-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: " + (selected ? "#4f46e5" : "#1e293b") + ";");
                descLabel.setStyle("-fx-font-size: 10px; -fx-text-fill: " + (selected ? "#6366f1" : "#64748b") + ";");
            }
        };

        cb.selectedProperty().addListener((o, ov, nv) -> updateStyle.run());
        updateStyle.run();
        styleUpdaters.add(updateStyle);
    ...
    private final java.util.List<Runnable> styleUpdaters = new java.util.ArrayList<>();

    public void refreshStyles() {
        for (Runnable r : styleUpdaters) {
            r.run();
        }
    }
